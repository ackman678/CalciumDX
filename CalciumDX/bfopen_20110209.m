...
Revision history---
James Ackman, 2011-02-09.  Added a new cell array in the results array that returns the channel label as a number. This can be used for separating channels in downstream applications.
%}
...
%     imageList = cell(numImages, 2);
    imageList = cell(numImages, 3); %jba 2011-02-09
...
%         pix = img.getData.getDataBuffer().getData();
        arr = img.getData.getDataBuffer().getData();
%         arr = reshape(pix, shape)';
        % build an informative title for our figure
        label = id;
        chanlabel=1;  %jba 2011-02-09
        if numSeries > 1
            qs = int2str(s);
        ...
        if numImages > 1
            ...
            if sizeC > 1
                qc = int2str(zct(2) + 1);
                chanlabel=str2double(qc); %jba 2011-02-09
        ...
        imageList{i, 1} = arr;
        imageList{i, 2} = label;
        imageList{i, 3} = chanlabel; %jba 2011-02-09
        clear img
        clear pix
        clear arr
