function [stn, decpt] = calciumdxdetartifacts_positive(x,nt)
% num=50; %testing
...
figure;  %testing
ax(1) = subplot(3,1,1); %testing
imagesc(nt) %testing
ax(2) = subplot(3,1,2); %testing
plot(x1); %testing
ax(3) = subplot(3,1,3); %testing
plot([x 0]); %testing
hold on; %testing
linkaxes(ax,'x');
...
%         ps = find(tr>baseavg); %look here for adjustment %problem is in baseavg
        ps = find(tr > -(avgchange+stdchange)); %this one is great for data with no baseline like detection on diff(data)
...
%More new stn(c) refinement-----------------------------------------------------
for c = 1:length(stn)
%         ps = find(tr > (avgchange+stdchange)); %this one is great for data with no baseline like detection on diff(data)
        ps = 1:length(tr);
        ps = ps(ps < stn(c)); %look here for adjustment
%         disp(ps)
        f = find(ps > (stn(c)-6));
%         disp(f)
        if ~isempty(f)
            f1 = find(tr(f) < (avgchange+stdchange));
%             disp(f)
            if ~isempty(f1)
                f2 = f(f1);
            stn(c) = f2(end);
...
for c = 1:length(stn)
    ...
    th = -(avgchange+stdchange); %look here for adjustment %problem is in baseavg
