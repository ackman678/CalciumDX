%myMakeContourMovieWavesOnsets
function A = myMakeContourMovieWavesOnsets(fnm,region,locationIndex,includebaseline,includecolorbar)
%locationIndex is an integer specifying which region.location you want to analyze. This assumes that the 'region' passed to the program has already been filtered through the 'splitregion.m' function.
...
%START Make Movie----------------------------------------------------------
if nargin < 4, includebaseline='false'; end
if nargin < 5, includecolorbar='false'; end
if nargin < 3 || isempty(locationIndex), locationIndex = []; splitReg = 0; end
if numel(locationIndex) == 1
splitReg = 1;
end
        ...
        for d=1:length(region.onsets{c})
            ...
            if ~isempty(ind)
                if region.onsets{c}(d) >= region.wavedata{locationIndex}.waveonsets(ind(end)) && region.onsets{c}(d) <= region.wavedata{locationIndex}.waveoffsets(ind(end))
                    ntSpk(c,region.onsets{c}(d)) = abs(region.onsets{c}(d)-region.wavedata{locationIndex}.waveoffsets(ind(end)))/abs(region.wavedata{locationIndex}.waveonsets(ind(end))-region.wavedata{locationIndex}.waveoffsets(ind(end)));
                end
            end
        end
    end
...
for c = 1:size(ntSpk,1)
    for findSpks = find(ntSpk(c,:))
        in = find(abs((1:size(ntSpk,2))-findSpks)<=1);  %smooth the spikes with +/- 1 frame signal
