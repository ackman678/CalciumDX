function [s, d] = calciumdxEvent_DetSingTrHannFast(tr,nn,period,sss,ddd,region)
...
if max(stdDevTm)>min(stdDevTm)*1.5 & size(tr,2)>=2000
    regSiz=1000;
    regNum=[1:regSiz:size(tr,2)];
    nstd=3;  % soglia x stimare rumore derivata  %x noise threshold estimate derived
    stdTh=5; % soglia su derivata %threshold on derivative
else
    regSiz=size(tr,2);
    regNum=[1:regSiz:size(tr,2)];
    nstd=3;  % soglia x stimare rumore derivata
...
%                         plot(intHist,ny,'r-')
        %
        [spkVetHelp, sh, dh, minSpk]= spkCalc_extract(trDif,i:i+regSiz-2,3,mu,sigma,(stdTh+addstd));
