function [data, responseArray, latencyArray, results2] = batchFetchStimResponseProps(filelist,region,stimuliIndices,bn,nstimuliSelect,limitToWaves)
...
results2={'filename' 'matlab.filename' 'region.name' 'roi.number' 'nrois' 'roi.height.px' 'roi.width.px' 'xloca.px' 'yloca.px' 'xloca.norm' 'yloca.norm'  'stimulus.desc' 'nstimuli' 'activeResponseFraction.region' 'responseFreq.norm' 'absoluteFiringFreq.Hz' 'meanLatency' 'eventNumber'};
else
...
results2={'matlab.filename' 'region.name' 'roi.number' 'nrois' 'roi.height.px' 'roi.width.px' 'xloca.px' 'yloca.px' 'xloca.norm' 'yloca.norm'  'stimulus.desc' 'nstimuli' 'activeResponseFraction.region' 'responseFreq.norm' 'absoluteFiringFreq.Hz' 'meanLatency' 'eventNumber'};
end
...
%----END fix files------------------------------------------------------------------ 
    ...
    [histResults,responseArray,responseFreq,latencyArray] = myMakeMultiPETHplot(region,stimuliToPlot,bn,nstimuli,1);
...
%    disp(size(eventIndices))    
    ...
    for eventIdx = 1:nEvents
		tmp = [rowinfo region.name{region.location(roiIND)} {roiIND nrois ROIsize(1) ROIsize(2) xlocapx ylocapx xlocanorm ylocanorm} region.stimuli{numStim}.description {numel(nstimuli) actvResponseFraction{region.location(roiIND)} responseFreq{numStim}(roiIND) absFreq}];
		tmpinfo = [tmpinfo; tmp;]; 
    end    
